...
namespace BizBook.Models
{
    public class CustomerClaimsDatabaseHelper
    {
        ...
        //--------------------------------------------------------------------------
        public Entities.CustomerClaims UpdateClaimAmount(Entities.CustomerClaims obj)
        {
            var claim = database.CustomerClaimsDbSet.Where(s => s.CId == obj.CId).FirstOrDefault();
            var payment = database.PaymentsDbSet.Where(s=> s.CustomerClaimsID == obj.CId).FirstOrDefault();
            claim.TotalPrice = obj.TotalPrice;
            payment.Credit = obj.TotalPrice;
            database.Entry(claim).State = EntityState.Modified;
