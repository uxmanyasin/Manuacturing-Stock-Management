...
namespace BizBook.Models
{
    public class PaymentsDatabaseHelper
    {
        ...
        public List<Payments> GetDescribedPayments(string statement, string type)
        {
            var listOfPayments = database.PaymentsDbSet.Where(s => s.Description.Contains(statement) && s.Type == type).ToList();
            return listOfPayments;
        }
        ...
        public List<Models.DummyModels.PaymentsListModel> CreateMultiplePayments(List<Models.DummyModels.PaymentsListModel> _list , bool isCustomer)
        {
            if (isCustomer is true) 
            {
                foreach (var item in _list)
                {
                    Entities.Payments payment = new Payments 
                    {
                        CustomerID = item.Id,
                        Debit = item.Debit,
                        Credit = item.Credit,
