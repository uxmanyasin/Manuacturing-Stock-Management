...
namespace BizBook.Models
{
    public class Sale_DetailsDatabaseHelper
    {
        ...
        public Entities.Sale UpdateSaleAmount(Entities.Sale obj)
        {
            Entities.Sale sale = database.SaleDbSet.Where(s=> s.SId == obj.SId).FirstOrDefault();
            sale.TotalPrice = obj.TotalPrice;
            sale.TotalProducts = obj.TotalProducts;
            sale.TotalQuantity = obj.TotalQuantity;
            var payment = database.PaymentsDbSet.Where(s => s.SaleId == obj.SId).FirstOrDefault();
            payment.Debit = sale.TotalPrice;

            database.Entry(sale).State = EntityState.Modified;
            database.Entry(payment).State = EntityState.Modified;
            database.SaveChanges();
            return obj;
        }
    }
